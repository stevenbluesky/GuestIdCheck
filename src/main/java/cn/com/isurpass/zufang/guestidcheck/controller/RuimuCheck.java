...
public class RuimuCheck 
{
    ...
    public Object ruimurequest(HttpServletRequest request,HttpServletResponse response)
    {
		...
		if ( StringUtils.isEmpty(command))
			return new RuimuIdCheckResponse(getCommandId(json) , -1 , "Command is null"); 
		
		if ( "record".equals(command))
			return guestchecked(json);
		else if ( "gettask".equals(command))
			return heartbeat(json);
		
		return new RuimuIdCheckResponse(getCommandId(json) , -2 , "Not support command:" + command );
    }
	...
	public static void savepic(String base64pic)
	{
		
		byte[] b = Base64.decodeBase64(base64pic);
		
		try {
			FileUtils.writeByteArrayToFile(new File("E:/tmp/ruimu.jpg"), b);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
